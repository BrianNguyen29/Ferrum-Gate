#!/usr/bin/env bash
# s7-sqlite-contention.sh — POST /v1/provenance/ingest concurrent writes
...
echo "───────────────────────────────────────────────────────────────"
echo "  SCENARIO: sqlite-contention  (workers=$WORKERS, duration=${DURATION}s)"
echo "───────────────────────────────────────────────────────────────"
echo "  NOTE: High concurrency tests SQLite write lock contention"
echo ""
            ...
            json_body=$(cat <<EOF
{
  ...
  "description": "Concurrent write test event from worker $worker_id",
  "execution_id": null,
  "intent_id": null,
  "trust_labels": [],
  "sensitivity_labels": [],
...
total=$(wc -l < "$TMPFILE")
errors=$(grep -v -E " (200|201|400|422|500|503)$" "$TMPFILE" | wc -l || true)
errors=${errors:-0}
expected=$(grep -c -E " (400|422|500|503)$" "$TMPFILE" || true)
expected=${expected:-0}
...
echo "  Expected:     $expected (400/422/500/503 validation/backpressure/degraded, ${expected_pct}%)"
if [[ "$conn" -gt 0 ]]; then
    echo "  Conn/000:     $conn"
fi
...
echo ""
echo "  NOTE: High p90/p99 latency may indicate SQLite write lock contention"
echo ""
