...
if [[ $FAILED -eq 0 ]]; then
    ...
    echo ""
    echo "NEXT STEPS (operator-owned, per docs/guides/operator.md and docs/PRODUCTION_NOTES.md):"
    echo "  1. Complete target environment adaptation using docs/guides/operator.md"
    ...
    echo "  6. Address remaining production blocker: real owned domain per Block A in docs/PRODUCTION_NOTES.md"
    echo ""
    echo "G2 evidence and operator signoff per docs/releases/v0.1.0-rc1.md for conditional single-node SQLite pilot scope only."
    echo "G2 remains CONDITIONAL. Block A remains WAIVED/CONDITIONAL; Blocks B/C are CLOSED."
