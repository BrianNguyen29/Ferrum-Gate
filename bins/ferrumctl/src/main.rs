...
struct ReadinessReport {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    slo_window: Option<slo_window::SloWindowState>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
async fn build_readiness_report(
    ...
    // SLO window state
    let window_path = slo_window::slo_window_state_path(window_dir.clone());
    if window_path.exists() {
        match slo_window::read_slo_window_state(&window_path) {
            Ok(mut state) => {
        ...
        Command::Evidence { sub } => match sub {
            ...
            EvidenceCommand::SloWindow { sub } => match sub {
                SloWindowCommand::Start { window_dir, notes } => {
                    slo_window::run_slo_window_start(window_dir, notes)?;
                }
                SloWindowCommand::Status { window_dir, json } => {
                    slo_window::run_slo_window_status(window_dir, json)?;
                }
                SloWindowCommand::Finalize {
                    ...
                    slo_window::run_slo_window_finalize(window_dir, notes, allow_early)?;
                }
            },
        },
...
"#;
    ...
    async fn test_readiness_report_offline_mode_with_local_state() {
        ...
        slo_window::run_slo_window_start(Some(dir.path().to_path_buf()), Some("test".to_string()))
            .unwrap();
