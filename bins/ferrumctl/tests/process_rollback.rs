...
/// Helper: seeds an execution and leaves it in Running state.
/// Returns the execution_id.
/// The execution is in Running state after this (execute was called, but not verify).
...
/// Test: pause-execution via CLI on a running execution succeeds.
#[tokio::test]
async fn test_process_pause_execution_via_binary() {
    let (temp_dir, runtime, _store) = create_test_runtime().await;
    let _ = &temp_dir;
    ...
    assert!(
        output.status.success(),
        "pause-execution should succeed. stderr: {}",
        String::from_utf8_lossy(&output.stderr)
    );

    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("paused") || stdout.contains("Paused") || stdout.contains("pause"),
        "stdout should mention paused. stdout: {}",
        stdout
    );

    // Verify execution is now Paused via API
    let resp = client
        .get(&format!("{}/v1/executions/{}", server_url, execution_id))
        .send()
        .await
    ...
    assert!(
        ...
        "execution state should be Paused after CLI pause, got {}",
        state
    );
...
/// Test: pause-execution via CLI fails when execution is in Prepared state.
#[tokio::test]
async fn test_process_pause_execution_from_prepared_conflict() {
    let (temp_dir, runtime, _store) = create_test_runtime().await;
    let _ = &temp_dir;
...
/// Test: pause-execution via CLI fails when execution does not exist.
#[tokio::test]
async fn test_process_pause_execution_not_found() {
    let (temp_dir, runtime, _store) = create_test_runtime().await;
    let _ = &temp_dir;
    ...
    let output = run_ferrumctl(
        &server_url,
        ["pause-execution", &fake_execution_id.to_string(), "--json"],
    )
    .await;
