//! Process-level integration tests for ferrumctl inspect-capability command.
//!
...
//!
//! Coverage:
//! - `server inspect-capability` — inspect a capability via CLI
//!
//! Each test:
...
// ---------------------------------------------------------------------------
// Test runtime bootstrap (reused from process_revoke_capability.rs)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// ferrumctl binary wrapper (reused from process_revoke_capability.rs)
// ---------------------------------------------------------------------------
...
/// Test: `ferrumctl server inspect-capability` returns capability details and exits with code 0.
#[tokio::test]
async fn test_process_inspect_capability_via_binary() {
    ...
    // Spawn ferrumctl inspect-capability
    let output = run_ferrumctl(
        &server_url,
        ["inspect-capability", &capability_id.to_string(), "--json"],
    )
    .await;
    ...
    // Verify output mentions the capability_id
    ...
    // Verify output contains expected capability fields
    assert!(
        stdout.contains("intent") || stdout.contains("proposal"),
        "stdout should contain intent or proposal field. stdout: {}, stderr: {}",
        stdout,
        stderr
    );
...
/// Test: `ferrumctl server inspect-capability` fails for non-existent capability.
#[tokio::test]
async fn test_process_inspect_capability_not_found() {
    let (temp_dir, runtime, _store) = create_test_runtime().await;
    let _ = &temp_dir;
    ...
    assert!(
        ...
        "stderr should mention not found or capability. stderr: {}",
        stderr
    );
...
/// Test: `ferrumctl server inspect-capability` fails for invalid UUID.
#[tokio::test]
async fn test_process_inspect_capability_invalid_uuid() {
    let (temp_dir, runtime, _store) = create_test_runtime().await;
    let _ = &temp_dir;
    ...
    let output = run_ferrumctl(
        &server_url,
        ["inspect-capability", "not-a-valid-uuid", "--json"],
    )
    .await;
