...
- **Production posture**: `production-ready = NO`; full G2 = `NOT COMPLETE`; PostgreSQL production = `NO`; HA/multi-node production HA = `NO`. Phase 9 manual multi-host drills, detection-only watchdog, GCP fencing-mechanism evidence, and one bounded operator-controlled fenced failover with host B app/PgBouncer redundancy exist as of 2026-05-27, but unattended automated/fenced production HA remains incomplete.
...
| 15 | SEC-6 minimal append-only audit log | Engineering + Operator | ✅ IMPLEMENTED / SIGNED — domain types, SQLite/Postgres repo, migration 008, `GET /v1/admin/audit-logs`, best-effort append, `ferrumctl admin audit list`; operator evidence review signed 2026-05-27 | Best-effort only; not compliance-grade; not production-ready | `docs/implementation-path/artifacts/2026-05-21-sec6-audit-log-implementation-evidence.md` + `docs/implementation-path/artifacts/2026-05-27-phase4-security-operator-signoff.md` | Future work: compliance-grade audit logging only if required |
| 16 | PG-2.3b reconnect / circuit breaker | Engineering | 📝 DEFERRED — docs-only rationale; `sqlx::PgPool` transparent reconnect sufficient for single-node | Revisit only after HA ADR is approved by operator and topology is defined | `docs/implementation-path/artifacts/2026-05-21-pg-2.3b-reconnect-circuit-breaker-backlog.md` | Engineering to revisit after HA ADR operator approval and topology decision |
| 17 | HA Phase 9 planning/manual evidence | Engineering + Operator | ✅ MANUAL MULTI-HOST + DETECTION + FENCING + HOST B REDUNDANCY EVIDENCE CAPTURED — HA ADR signed by operator delegate 2026-05-21; prerequisites unblocked 2026-05-27; topology selected; 4 manual multi-host drills captured with failback, RTO/RPO log, observed RPO 0 marker loss, bounded monitoring evidence, automated failover/fencing ADR drafted, detection-only watchdog installed/verified, config parity normalized, Alertmanager unit mismatch resolved, GCP fencing script created/validated, app-host guard verified, standby host B fenced/recovered, and one bounded operator-controlled host-A-fenced drill with host B PgBouncer/ferrumd readiness captured | Multi-host production HA = NO; Phase 9 HA-4 unattended automated/fenced failover still required before any production HA claim; external endpoint cutover, repeated unattended drills, and full human incident-response signoff remain incomplete | `docs/implementation-path/artifacts/2026-05-21-ha-phase-9-planning-backlog.md` + `docs/implementation-path/artifacts/2026-05-27-ha-phase9-prerequisites-unblocked.md` + `docs/implementation-path/artifacts/2026-05-27-ha-phase9-multihost-topology-adr.md` + `docs/implementation-path/artifacts/2026-05-27-ha-phase9-multihost-drill-evidence.md` + `docs/implementation-path/artifacts/2026-05-27-ha-phase9-automated-failover-fencing-adr.md` + `docs/implementation-path/artifacts/2026-05-27-ha-phase9-watchdog-config-parity-evidence.md` + `docs/implementation-path/artifacts/2026-05-27-ha-phase9-gcp-fencing-evidence.md` + `docs/implementation-path/artifacts/2026-05-27-ha-phase9-host-b-redundancy-fenced-drill-evidence.md` | Next: automate the currently manual sequence only after external endpoint/routing strategy and operator incident-response signoff are defined; repeat unattended/fenced drills before any HA-4 claim |
| 18 | SLO sustained dry-run rehearsal | Engineering | ✅ LOCAL REHEARSAL — `make slo-sustained-dry-run` passed; 5 samples, 100% availability, avg latency 42 ms, ~4 min duration | NOT valid SLO evidence; NOT sustained window; NOT production claim | `docs/implementation-path/artifacts/2026-05-28-slo-dry-run-rehearsal-evidence.md` | Operator to execute real sustained window when ready |
| 19 | HA local rehearsal after script fix | Engineering | ✅ LOCAL REHEARSAL — psql forced to TCP `-h localhost`, `listen_addresses='*'` simplified; failover drill 16/16 pass (RTO 4s, RPO 0); ferrumd reconnect drill 13/13 pass (app-level RTO 4s) | NOT production HA; NOT automated failover; HA-4 remains NOT COMPLETE | `docs/implementation-path/artifacts/2026-05-28-ha-local-rehearsal-and-script-fix-evidence.md` | Script fix to be committed when operator ready |
| 20 | Delegated ship-fast waiver signoff | Engineering + Operator (delegated) | ✅ SIGNED — operator-delegated risk acceptance for rehearsal evidence | Waiver is risk acceptance, not evidence completion; HA-4 / sustained SLO / multi-host production HA remain NOT COMPLETE | `docs/implementation-path/artifacts/2026-05-28-delegated-ship-fast-waiver-signoff.md` | No further action unless operator revokes waiver |
...
| [`docs/production-readiness-v2/`](../production-readiness-v2/) | P1 planning docs (SLO/SLA, PG hardening, MCP target, security/tenant ADR, evidence checklist, blockers plan) |
...
- **NOT production K8s/HA**: Helm live install verified on local kind cluster only; one manual multi-host PostgreSQL drill does not make the system production HA.
- **NOT unattended automated/fenced failover complete**: GCP fencing script exists, standby B fencing passed, and one operator-controlled host-A-fenced drill with host B redundancy passed. The sequence is not unattended automation and does not include external endpoint cutover or production signoff.
