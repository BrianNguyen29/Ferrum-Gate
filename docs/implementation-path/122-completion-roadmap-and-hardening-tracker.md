...
| 17 | HA Phase 9 planning | Engineering + Operator | 📝 DRAFT EXISTS — HA ADR drafted and operator-reviewable; preconditions (PG stable, security model, SLO metrics, backup evidence) partially met | HA/multi-node = NO; do not start implementation until operator approves ADR and preconditions closed | `docs/implementation-path/artifacts/2026-05-21-ha-phase-9-planning-backlog.md` | Operator reviews HA ADR; engineering revisits after operator approval |
