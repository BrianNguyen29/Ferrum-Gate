...
- `ferrum-gateway` — full orchestration: evaluate -> mint -> authorize -> prepare -> execute -> verify -> compensate (internal: commit/rollback as orchestration semantics); negative paths: deny, quarantine, RequireApproval, draft-only gated at evaluate (before prepare); U1-S2 verify-time outcome assessment annotation (annotate-only, does not change verify decision semantics); U1-S5a soft gate preview DONE at prepare-time (emit warn signals does not block state-machine or change auto-commit); U1-S5b hard gate TODO (block at prepare-time, state-machine halts, auto-commit unchanged per R3 contract semantics)
