...
| **Phase 3** | PostgreSQL migration | P4.1–P4.3 complete; P4.4/P5 deferred | Target: 1000+ writes/s, 200+ pipelines/s |
...
- PostgreSQL local Docker/runtime support is complete (P4.1–P4.3). `postgres://` and `postgresql://` DSNs are accepted when built with `--features postgres`. Production deployment and data migration (P4.4) remain deferred.
