...
| **Phase 3** | PostgreSQL migration | P4.1–P4.4 MVP complete; P5 production readiness deferred | Target: 1000+ writes/s, 200+ pipelines/s |
...
- PostgreSQL local Docker/runtime support is complete (P4.1–P4.3). `postgres://` and `postgresql://` DSNs are accepted when built with `--features postgres`. P4.4 MVP complete (dry-run default, --apply, empty-target safety, count+ID validation); P5 production readiness (HA, multi-node, operator signoff) deferred.
