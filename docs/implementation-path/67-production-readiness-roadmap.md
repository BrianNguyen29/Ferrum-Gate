...
|---|---------|-------|---------------|------------|
| 1 | **Real owned domain** (Block A) | Operator | WAIVED/CONDITIONAL for single-node SQLite pilot only — DuckDNS accepted by operator on 2026-05-17 as proxy; real owned domain still required for production-ready or full G2 closure | P0 — before any external exposure; waived for conditional pilot |
...
**Current state**: VM uses DuckDNS (`ferrumgate.duckdns.org`). DuckDNS is accepted by the operator on 2026-05-17 for the conditional single-node SQLite pilot but is **not** a production-owned domain. Block A is **WAIVED/CONDITIONAL** for the pilot only. A real owned domain is still required for production-ready posture or full G2 closure.
...
**Current state**: Prometheus + AlertManager active. SendGrid API key secret present on VM. AlertManager config contains SendGrid webhook receiver. Synthetic alert POST returned HTTP 200 and was visible in AlertManager API. Operator confirmed receipt of inbox-check alert `TEST_ID=fg-inbox-check-20260516-052910` for at least one contact, with email content matching subject `FerrumGate Alert: FerrumGateInboxDeliveryCheck`, status `resolved`, severity `warning`, service `ferrumgate`. Secondary contact was configured in active AlertManager config (`/etc/prometheus/alertmanager.yml`) after discovering the service loads that path; active config validation showed `ACTIVE_CONFIG_CHECK=PASS`, `ALERTMANAGER_SERVICE=active`, `ACTIVE_SECONDARY_PRESENT=YES`, `ACTIVE_EMAIL_TO_COUNT=4`. Secondary test alert evidence: `SECONDARY_TEST_ID=fg-secondary-check-20260516-153221`, `SECONDARY_START_AT_UTC=2026-05-16T15:32:21Z`, `ALERT_POST_HTTP=200`, `ALERT_VISIBLE=YES`; operator confirmed secondary inbox receipt. Bearer token rotation executed on VM (new token 200, old token 401, ROTATION_RESULT=PASS; token generated on VM and never printed). SendGrid API key rotation verified on VM: active secret path `/etc/ferrumgate/secrets/sendgrid-api-key` has `MODE=640 OWNER=root:prometheus`, directory permissions corrected to `750 root:prometheus`, synthetic alert `FerrumGateSendGridDirPermFix` delivered to primary and secondary inboxes, old key revoked/deleted, and SSH firewall restored to `118.69.4.63/32`. See [`artifacts/2026-05-17-sendgrid-rotation-evidence.md`](./artifacts/2026-05-17-sendgrid-rotation-evidence.md).
...
|------|----------|--------|
| G-B1 | Test alert delivered to primary operator contact | ✅ Done — operator confirmed primary inbox receipt of `TEST_ID=fg-inbox-check-20260516-052910`; email content verified (subject `FerrumGate Alert: FerrumGateInboxDeliveryCheck`, status `resolved`, severity `warning`, service `ferrumgate`) |
| G-B2 | Test alert delivered to secondary contact | ✅ Done — active AlertManager config validated (`ACTIVE_CONFIG_CHECK=PASS`, `ALERTMANAGER_SERVICE=active`, `ACTIVE_SECONDARY_PRESENT=YES`, `ACTIVE_EMAIL_TO_COUNT=4`); synthetic alert posted (`ALERT_POST_HTTP=200`, `ALERT_VISIBLE=YES`, TEST_ID `fg-secondary-check-20260516-153221`, START_AT_UTC `2026-05-16T15:32:21Z`); operator confirmed secondary inbox receipt |
| G-B3 | Key rotation procedure executed at least once in non-prod | ✅ Done — bearer token rotation executed on VM; SendGrid API key rotation verified on VM with active secret path permissions fixed, primary+secondary delivery confirmed, and old key revoked/deleted. See [`artifacts/2026-05-17-sendgrid-rotation-evidence.md`](./artifacts/2026-05-17-sendgrid-rotation-evidence.md). |
| G-B4 | Escalation matrix documented and populated for primary+secondary email path | ✅ Done — operator formally acknowledged primary/secondary contact paths and timeout targets on 2026-05-17; SMS/webhook deferred. See [`artifacts/2026-05-17-escalation-matrix-acknowledgment.md`](./artifacts/2026-05-17-escalation-matrix-acknowledgment.md). |
...
- [`artifacts/2026-05-17-sendgrid-rotation-evidence.md`](./artifacts/2026-05-17-sendgrid-rotation-evidence.md) §SendGrid rotation, permission fix, delivery verification, old-key revocation, SSH firewall restoration.
...
| 2026-05-16 (secondary contact confirmation) | Operator confirmed secondary inbox receipt for test alert `TEST_ID=fg-secondary-check-20260516-153221`. Active AlertManager config (`/etc/prometheus/alertmanager.yml`) validated with `ACTIVE_CONFIG_CHECK=PASS`, `ALERTMANAGER_SERVICE=active`, `ACTIVE_SECONDARY_PRESENT=YES`, `ACTIVE_EMAIL_TO_COUNT=4`. Secondary contact delivery confirmed (G-B2). Escalation matrix populated enough for primary+secondary email path (G-B4). Block A remains blocked. SendGrid API key rotation remains pending. No production-ready claim preserved. | Engineering |
| 2026-05-16 (SendGrid verification) | VM-side SendGrid rotation verification did NOT confirm rotation: `SENDGRID_SECRET_NONEMPTY=YES`, `SENDGRID_BACKUP_COUNT=0`, `SENDGRID_SECRET_MTIME_UTC=2026-05-10 04:58:58.710517174 +0000`. Explicitly recorded as pending/not VM-verified across all docs. Block A remains blocked. No production-ready claim preserved. | Engineering |
...
| 2026-05-21 (canonical SLO + Helm + conditional signoff) | Added canonical SLO certification evidence: Run #1 default FAIL (46.767% 429), Run #2 tuned FAIL (73.445% 429), Run #3 max-valid PASS (0% 429). Live Helm install on kind passed. Target MCP smoke 15/15 passed. PostgreSQL scoped token repo implemented. BrianNguyen authorized conditional re-signoff for single-node SQLite pilot scope. Documented SLO default-config gap decision: default/tuned configs insufficient; max-valid config required. Updated cross-reference index with `2026-05-21-canonical-slo-helm-conditional-signoff.md` and `2026-05-21-target-slo-mcp-helm-domain-evidence.md`. No production-ready claim preserved. | Engineering |
| 2026-05-21 (SEC-6 + backlog artifacts) | Implemented SEC-6 minimal append-only audit log: domain types, SQLite/Postgres `AuditLogRepo`, migration 008, `GET /v1/admin/audit-logs` with `admin:audit` scope, best-effort append for token/policy/approval/execution actions, `ferrumctl admin audit list`. Verification passed (fmt, check, clippy, workspace tests, postgres feature tests). Created dated evidence artifact. Added concise planning backlog artifacts for PG-2.3b reconnect/circuit breaker and HA Phase 9. Updated `10-evidence-checklist.md`, `11-blockers-and-unblock-plan.md`, `ROADMAP.md`, `122-completion-roadmap-and-hardening-tracker.md`. No production-ready claim preserved. Block A remains WAIVED/CONDITIONAL. | Engineering |
...
*Next update: Block A is WAIVED/CONDITIONAL — DuckDNS accepted by operator on 2026-05-17 for single-node SQLite pilot only; real owned domain still required for production-ready or full G2 closure. Block B is CLOSED. 2026-05-21 conditional re-signoff authorized by BrianNguyen: canonical SLO Run #3 max-valid PASS (0% 429), live Helm kind install PASS, target MCP smoke 15/15 PASS. PostgreSQL scoped token repo implemented and passing. SLO default-config gap documented: default/tuned configs FAIL; max-valid config required. Engineering owner to investigate; operator to ratify config choice. `cargo-deny v0.19.6` + `cargo-audit v0.22.1` installed with `make audit` passing. `RUSTSEC-2023-0071` ignored as uncompiled optional dependency (`rsa` via `sqlx-mysql`, blocked by `default-features = false` on `sqlx`).*
