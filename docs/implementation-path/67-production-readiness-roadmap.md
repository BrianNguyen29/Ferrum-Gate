...
| P0.4 | **Backup automation partially configured** | Operator | External scheduler (cron/systemd timer) configured; `ferrumctl backup verify` passes; **retention pruning not verified** | 🟡 Partial (operator-owned; retention pruning not verified; not full production signoff) |
...
| P2.4 | **TLS/reverse proxy partially configured** | Operator | TLS termination at reverse proxy (nginx/etc.); ferrumd does not terminate TLS; HTTPS probes pass through proxy | 🟡 Partial (operator-owned; not full production signoff) |
| P2.5 | **Bearer token partially configured** | Operator | Real bearer token generated by operator via `openssl rand -hex 32`; auth_mode=bearer confirmed | 🟡 Partial (operator-owned; not full production signoff) |
...
| Backup automation | Operator | Operator | 🟡 Partial — timer active, backups verified; **retention pruning not verified** |
| Restore drill | Operator | Operator | ✅ Passed 2026-05-15 — fixed binary deployed; 0.463s restore; live DB verify OK |
| TLS/reverse proxy | Operator | Operator | 🟡 Partial — HTTPS probes pass; operator-independent cert-path verification not done |
...
| 2026-05-15 | Reconciled with latest evidence: P0.2 D1–D6 passed 6/6 on 2026-05-13; P0.3 restore drill passed on 2026-05-15; P0.4 retention pruning remains not verified; P2.4/P2.5 remain partial. No production-ready claim preserved. | Engineering |
...
*Next update: Operator-owned target execution evidence provided for D1–D6 (2026-05-13) and restore drill (2026-05-15). Backup automation retention pruning remains not verified. Engineering items tracked in `11-remaining-tasks.md`.*
