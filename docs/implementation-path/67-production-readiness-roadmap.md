...
| G-B4 | Escalation matrix documented and populated for primary+secondary email path | 🟡 Populated — primary and secondary email contacts configured in active AlertManager config; skeleton documented; full operator acknowledgment and additional channels (SMS/webhook) remain pending |
...
*Next update: Block A remains blocked — operator confirmed no real domain and no DNS available yet. Block B: operator confirmed primary inbox receipt of `TEST_ID=fg-inbox-check-20260516-052910` with subject/content verified (G-B1 done); secondary contact delivery confirmed (G-B2) with `TEST_ID=fg-secondary-check-20260516-153221`; bearer token rotation executed on VM; SendGrid API key rotation verified on VM with synthetic alert `FerrumGateSendGridDirPermFix` delivered to primary+secondary inboxes and old key revoked/deleted (G-B3 done); escalation matrix populated enough for primary+secondary email path but formal acknowledgment remains pending (G-B4 partial). Engineering items 7–9 closed: ferrum-cap fix verified, local audit gate added, and `cargo-deny v0.19.6` + `cargo-audit v0.22.1` installed with `make audit` passing. `RUSTSEC-2023-0071` ignored as uncompiled optional dependency (`rsa` via `sqlx-mysql`, blocked by `default-features = false` on `sqlx`).*
