...
*Next update: Block A is WAIVED/CONDITIONAL — DuckDNS accepted by operator on 2026-05-17 for single-node SQLite pilot only; real owned domain still required for production-ready or full G2 closure. Block B is CLOSED: operator confirmed primary inbox receipt of `TEST_ID=fg-inbox-check-20260516-052910` with subject/content verified (G-B1 done); secondary contact delivery confirmed (G-B2) with `TEST_ID=fg-secondary-check-20260516-153221`; bearer token rotation executed on VM; SendGrid API key rotation verified on VM with synthetic alert `FerrumGateSendGridDirPermFix` delivered to primary+secondary inboxes and old key revoked/deleted (G-B3 done); escalation matrix formally acknowledged on 2026-05-17 (G-B4 done). Engineering items 7–9 closed: ferrum-cap fix verified, local audit gate added, and `cargo-deny v0.19.6` + `cargo-audit v0.22.1` installed with `make audit` passing. `RUSTSEC-2023-0071` ignored as uncompiled optional dependency (`rsa` via `sqlx-mysql`, blocked by `default-features = false` on `sqlx`).*
