...
| P1.4 | **Full metrics/observability** | Engineering | No latency histograms, no advanced Prometheus labels; baseline `/v1/metrics` exists with request counters, store health gauge, and write queue depth gauge | 🟡 Partial — `/v1/metrics` exists with: request counters per endpoint (healthz, readyz, readyz/deep, metrics), store health gauge (`ferrumgate_store_health_up`), SQLite write queue depth gauge (`ferrumgate_write_queue_depth`), governance error counters per route (20+ routes), governance success counters per route (20+ routes); missing: latency histograms and HTTP method/status labels |
...
- P1.4: `/v1/metrics` provides: request counters per endpoint (healthz, readyz, readyz/deep, metrics),
  ...
  `ferrumgate_governance_errors_total` per route (20+ governance endpoints), and
  `ferrumgate_governance_success_total` per route (20+ governance endpoints). Missing: latency histograms
  and HTTP method/status labels. Adding latency histograms would require wrapping with timing middleware — out
  of scope for a bounded change. Method/status labels would require per-handler instrumentation or a metrics
  middleware bridge from TraceLayer.
