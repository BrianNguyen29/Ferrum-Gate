...
| Sync-2: PF7 sync session tracking (stateful, not read-only) | P2 | Sync-1 impl | DONE | DB-backed session flag via `sync_state(id=1).sync_in_progress`; `SqliteSyncPreflightRepo::acquire_sync_session_async()` / `release_sync_session_async()` provide atomic compare-and-set semantics; `SyncSessionGuard` explicit-release helper in `ferrum-store/src/sync_service.rs` (does NOT auto-release on drop; see rationale in sync_service.rs docs); richer distributed session coordination remains deferred |
| Sync-3: live-readiness orchestration helper (`run_live_sync_readiness_once`) | P2 | Sync-3 impl | **DONE** | Internal orchestration helper composing PF7 acquire + live probe-to-cache + read-only readiness verdict + guaranteed release; `LiveSyncReadinessResult` / `LiveSyncReadinessError` with explicit `SessionReleaseFailed`; `run_live_sync_readiness_once` in `ferrum-store/src/sync_service.rs` |
...
| Sync-2: `LeaderAheadEmpty` variant removed | P3 cleanup | Sync-2 impl | DONE | Removed; `Bootstrap` covers the case |
...
| P2.4b | Analyze leader-election requirements for future implementation | **DONE** | Decision doc `27-p2-leader-election-requirements-analysis.md`: requirements (LE1-LE6, NI1-NI3, PR1-PR3), option comparison (Raft, External Coordinator, Custom, Multi-Paxos), Raft recommendation with rationale, minimal interface contract, and future Raft integration design doc prerequisite identified |

**Dependencies:** None (exploratory/analysis only).

**Touchpoints:**
- `docs/implementation-path/26-p2-sqlite-read-replica-use-cases.md`
- `docs/implementation-path/27-p2-leader-election-requirements-analysis.md`
- `docs/15-deployment-and-operations.md`
- `docs/implementation-path/18-cross-node-ledger-sync-plan.md`
...
| Consensus / leader election | Beyond P3 | Analysis complete; implementation deferred | P2.4b analysis recommends Raft; implementation remains post-P3 |
| HA / multi-leader replication | P2 | Analysis complete | SQLite read-replica use cases analysis DONE (`26-p2-sqlite-read-replica-use-cases.md`); leader-election analysis DONE (`27-p2-leader-election-requirements-analysis.md`); Raft recommended; full HA implementation post-P2 |
...
| Sync-2 PF2/PF6/PF7 sync state | `crates/ferrum-store/src/sqlite/sync_preflight.rs` + `migrations/003_add_sync_state.sql` | `sync_state` table (id=1, has_inflight_commits, has_uncommitted_entries, sync_in_progress); `read_local_state_async()` and `set_sync_flags_test_only()` in `SqliteSyncPreflightRepo`; `acquire_sync_session_async()` / `release_sync_session_async()` provide atomic compare-and-set session tracking for PF7; `SyncSessionGuard` explicit-release helper in `ferrum-store/src/sync_service.rs` (explicit `release().await` required; does NOT auto-release on drop) |
...
| Sync-3: live-readiness orchestration helper | `crates/ferrum-store/src/sync_service.rs` | `run_live_sync_readiness_once()`: internal helper composing PF7 acquire + live probe-to-cache + read-only readiness verdict + guaranteed release; `LiveSyncReadinessResult`, `LiveSyncReadinessError` with explicit `SessionReleaseFailed` variant; fail-closed; not a public sync API |
...
- P2.4b leader-election requirements analysis: `27-p2-leader-election-requirements-analysis.md`
