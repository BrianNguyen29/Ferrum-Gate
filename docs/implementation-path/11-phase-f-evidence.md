...
- runtime config docs va CLI/debug flow toi thieu da co them mot nhip thuc dung; TLS termination, capability persistence, va mot so release-checklist items khac van con mo
...
- Operator readiness: harden them TLS/ingress story va capability persistence cho rollout beyond single-process dev/prod-like use
