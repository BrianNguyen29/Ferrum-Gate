...
- `tests/integration_gateway_flow.rs`: happy path, deny, quarantine, rollback, compensate, approval, draft-only, single-use capability, scope mismatch deny, execution binding enforcement cho ca 5 loai `File`/`Http`/`Sqlite`/`Git`/`EmailDraft`, va fs/sqlite/maildraft/git/http-initial adapter-backed recovery evidence.
...
- rollback va compensate terminal recovery paths o muc gateway orchestration/state/provenance, va fs/sqlite/maildraft/git/http-initial adapter-backed recovery cho file create/delete + overwrite/restore row state + draft create/delete + git ref restore + HTTP GET no-op recovery
...
- Phase C: execution-time enforcement cho ca 5 loai da hoan tat; neu can harden tiep thi uu tien scope-subset edge cases va HTTP payload/adapter parity tiep theo
