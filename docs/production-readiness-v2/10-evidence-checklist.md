...
| 1.14 | PG-4a: Schema version table + idempotent runner (PG-4b.1 docs+runner cleanup done, PG-4b.2 bounded forward-only engine done, PG-4b.3 rollback strategy doc done; **CI drift gate prepared**) | Engineering | `pg-migration-evidence.md` + `02-postgres-production-plan.md` §PG-4b | ✅ COMPLETE — PG-4a done; PG-4b.1/4b.2/4b.3 done; **CI postgres feature gate added to `.github/workflows/ci.yml`** — explicit `cargo check` + `cargo clippy` with `--features postgres` on key crates (`ferrumd`, `ferrum-migrate`, `ferrum-store`, `ferrum-gateway`) enforce compile-time drift detection on every push/PR; `--all-features` also present as backup; live execution pending normal CI trigger |
...
| 2.17 | SLO sustained dry-run rehearsal — observation pipeline validation | Engineering | `docs/implementation-path/artifacts/2026-05-28-slo-dry-run-rehearsal-evidence.md` | ✅ LOCAL REHEARSAL — 5 samples, 100% availability, avg latency 42 ms, ~4 min duration; **NOT valid SLO evidence**; NOT sustained window; NOT production claim |
...
| 8.10 | DEP-10: Grafana dashboard template and import docs | Engineering | `configs/monitoring/ferrumgate-grafana-dashboard.json` + `configs/monitoring/README.md` §Grafana Dashboard | ✅ COMPLETE — dashboard JSON with generic PromQL panels; import via UI or provisioning; NOT production observability; metric names may need adjustment for actual instrumentation |

## Phase 9 — HA/multi-node

> **Status updated 2026-05-27**: Phase 9 prerequisites are unblocked, multi-host topology ADR is selected, 4 multi-host/operator-environment manual failover/failback drills have been captured, bounded monitoring readiness evidence exists, an automated failover/fencing ADR has selected detection-only/manual promotion, the detection-only watchdog has been installed/verified, GCP fencing-mechanism evidence exists, and host B PgBouncer/ferrumd redundancy was used in one bounded operator-controlled fenced failover drill. Unattended multi-host automated failover and production HA remain open.
...
| 9.3g | Host B PgBouncer/ferrumd redundancy and bounded fenced failover drill | Engineering + Operator | [`2026-05-27-ha-phase9-host-b-redundancy-fenced-drill-evidence.md`](../../implementation-path/artifacts/2026-05-27-ha-phase9-host-b-redundancy-fenced-drill-evidence.md) | ✅ OPERATOR-CONTROLLED FENCED DRILL — host A fenced with explicit force, B promoted and served readyz/deep in 69s, RPO 0, failback restored; NOT unattended HA-4 automation and no external endpoint cutover |
| 9.3h | HA local rehearsal after script fix — failover and ferrumd reconnect drills | Engineering | [`2026-05-28-ha-local-rehearsal-and-script-fix-evidence.md`](../../implementation-path/artifacts/2026-05-28-ha-local-rehearsal-and-script-fix-evidence.md) | ✅ LOCAL REHEARSAL — psql forced to TCP `-h localhost`, `listen_addresses='*'` simplified; failover drill 16/16 pass (RTO 4s, RPO 0); ferrumd reconnect drill 13/13 pass (app-level RTO 4s); NOT production HA; NOT automated failover |
...
- **NOT sustained SLO window**: SLO dry-run rehearsal (`2026-05-28-slo-dry-run-rehearsal-evidence.md`) is pipeline validation only, not a 7–30 day observation window.
...
- **NOT multi-host automated failover**: Phase 9 multi-host evidence uses manual/operator-controlled promotion, fencing, service activation, and reroute choices. No unattended multi-host automatic failover mechanism is complete.
- **NOT HA-4 unattended automated failover**: Local rehearsals after script fix (`2026-05-28-ha-local-rehearsal-and-script-fix-evidence.md`) remain manual/operator-controlled. HA-4 is deferred.
