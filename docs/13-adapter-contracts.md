...
- **H1.5a Retry/Backoff (delivered):** `execute` co retry/backoff cho transient errors (timeout, connection refused/reset). Bounded: 1 retry sau 100ms backoff. Khong retry non-retryable errors (validation errors, 4xx/5xx responses).
- **H1.5a Idempotency Key (delivered):** Mutation methods (POST/PUT/PATCH/DELETE) automatically receive `Idempotency-Key: fg-idempotency-<execution_id>` header de lam retries an toàn. `GET` không có idempotency key. Metadata `executed_idempotency_key_present` boolean追踪.
