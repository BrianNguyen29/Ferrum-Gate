...
> **⚠️ Status (updated 2026-05-27):** FerrumGate has Tier 1 **COMPLETE / ACKNOWLEDGED** and Tier 1.5 **COMPLETE / ACKNOWLEDGED** evidence. Tier 1.5 covers PostgreSQL target deployment, same-VM PostgreSQL primary/standby topology, and same-VM automated failover on the nonprod target VM. Do not claim production-ready: Tier 2 still requires a real owned domain, DNS, real-domain revalidation, sustained SLO window, full G2 re-signoff, and final production posture signoff. Multi-host production HA is not implemented. Default package version: `0.1.0`. Repository: `https://github.com/BrianNguyen29/Ferrum-Gate` (upstream/original — private, accessible with authorized GitHub credentials). See [00a-domainless-readiness-tier.md](./docs/production-readiness-v2/00a-domainless-readiness-tier.md) and [13-tier-1.5-completion-status.md](./docs/production-readiness-v2/13-tier-1.5-completion-status.md) for current status.
