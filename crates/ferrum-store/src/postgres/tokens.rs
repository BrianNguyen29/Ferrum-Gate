...
fn row_to_token(row: &sqlx::postgres::PgRow) -> Result<ScopedToken> {
...
impl TokenRepo for PostgresTokenRepo {
    ...
    async fn list(
        ...
        let mut next_idx = 1;
        if active_only {
            sql.push_str(" AND revoked_at IS NULL AND expires_at::timestamptz > now()");
        }
        if actor_id.is_some() {
            sql.push_str(&format!(" AND actor_id = ${}", next_idx));
            next_idx += 1;
        }
        if role.is_some() {
            sql.push_str(&format!(" AND role = ${}", next_idx));
            next_idx += 1;
        }
        sql.push_str(&format!(" ORDER BY created_at DESC LIMIT ${}", next_idx));
