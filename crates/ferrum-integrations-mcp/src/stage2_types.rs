...
//! - `IntentCompileResponse`: Re-exported from `ferrum_proto`; gateway response envelope with `intent_id` and warnings
//!
//! ## D1.3.1 Permitted
//!
...
// ---------------------------------------------------------------------------
// Client-Side Proposal ID Generation (D1.3.3+)
// ---------------------------------------------------------------------------

/// Generates a client-side proposal ID for governance pipeline evaluation.
///
/// Per doc 79 P3: `proposal_id` is generated by the MCP/client before
/// proposal evaluation, not parsed from the compile response. The real
/// gateway `IntentCompileResponse` (re-exported from `ferrum_proto`) contains
/// an `envelope` with `intent_id` and `warnings`, not a `proposal_id`.
///
/// This function is PURE — deterministic, no side effects, no I/O.
#[allow(dead_code)]
pub fn generate_proposal_id() -> ferrum_proto::ProposalId {
...
mod tests {
    ...
    fn test_generate_proposal_id_produces_uuid() {
        // Per doc 79 P3: proposal_id is client-generated before evaluation
        let id = generate_proposal_id();
        // ProposalId wraps Uuid which formats as 8-4-4-4-12 hex digits
        let id_str = id.to_string();
        assert!(id_str.len() == 36, "UUID should be 36 characters");
        assert!(
            id_str.chars().filter(|c| *c == '-').count() == 4,
