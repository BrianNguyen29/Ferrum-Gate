...
///
/// Per doc 75 §1.1: Returns `proposal_id` after successful compilation.
///
/// ## Example
///
/// ```json
...
/// # UNVERIFIED
///
/// This DTO is a D1.3.1 placeholder and does NOT match the real gateway
/// `IntentCompileResponse`, which is `{ envelope, warnings }` and contains
/// `envelope.intent_id` rather than `proposal_id`.
///
/// TODO(D1.3.3): replace or deprecate this placeholder before any real
/// `POST /v1/intents/compile` wiring. `proposal_id` must be generated by the
/// MCP/client before proposal evaluation, not parsed from compile output.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntentCompileResponse {
    /// Unique identifier for the compiled proposal.
    /// Used in subsequent calls to `POST /v1/proposals/{proposal_id}/evaluate`.
